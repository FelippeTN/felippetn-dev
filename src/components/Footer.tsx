    ...
    <footer className="relative overflow-hidden border-t border-line bg-bg-soft/45" ref={footer}>
      ...
      <div className="wrap reveal py-[clamp(48px,8vw,88px)]" ref={content}>
        <div className="grid gap-12 max-[640px]:gap-10 lg:grid-cols-[minmax(0,1.3fr)_minmax(240px,0.7fr)_minmax(220px,0.55fr)]">
          <div>
            <a href="#top" className="group inline-flex items-center gap-4" aria-label="Voltar ao topo">
              <span className="grid h-12 w-12 place-items-center rounded-full border border-line-strong transition-colors duration-300 group-hover:border-accent group-hover:bg-accent-soft">
            ...
            <ul className="grid gap-3">
              {navLinks.map((link) => (
                <li key={link.href}>
                  ...
                  >
                    <span className="h-px w-5 bg-line-strong transition-all duration-300 group-hover:w-8 group-hover:bg-accent" />
                    {link.label}
                  </a>
                </li>
              ))}
          ...
          <div>
            ...
            <a
              href={`mailto:${EMAIL}`}
              className="break-all text-sm text-muted transition-colors duration-300 hover:text-ink"
            >
              {EMAIL}
            </a>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-3 inline-flex items-center gap-2 text-sm text-muted transition-colors duration-300 hover:text-ink"
            >
        ...
        <div className="mt-14 flex flex-wrap items-center justify-between gap-4 border-t border-line pt-6 font-mono text-[11px] uppercase tracking-[0.14em] text-muted max-[640px]:mt-10 max-[640px]:items-start max-[640px]:tracking-[0.08em]">
          ...
          <a href="#top" className="group inline-flex items-center gap-2 transition-colors duration-300 hover:text-accent">
            Voltar ao topo
